...
with open(workdir + 'movielens-100k-train.txt')as fin:
    for line in fin:
        ...
        iid = int(line[1])
        r = float(line[2])
        # create item rating dict
        item_rating_dict[(uid,iid)] = r

        if r > 3.99:
            if uid in user_pos_train:
                user_pos_train[uid].append((iid,r))
            else:
    ...
    for line in fin:
        ...
        if r > 3.99:
            if uid in user_pos_test:
                user_pos_test[uid].append([iid,r])
            else:
...
def simple_test_one_user(x):
    ...
    tmpset = []
    for each in user_pos_train[u]:
        tmpset.append(each[0])

    test_items = list(all_items - set(tmpset))
    item_score = []
    for i in test_items:
...
def generate_for_d(sess, model, filename):
    ...
    for u in user_pos_train:
        ...
        for i in range(len(pos)):
            if item_rating_dict.get((u,neg[i])) is None:
                value = 2.5
            else:
                value = item_rating_dict.get((u,neg[i]))
            data.append(str(u) + '\t' + str(pos[i][0])+","+ str(pos[i][1]) + '\t' + str(neg[i])+","+str(value))
        ...
        if epoch >= 0:
            for d_epoch in range(2):
                ...
                while True:
                    if uid >= USER_NUM:
                        ...
                        input_user, input_item, input_label, index_new = ut.get_batch_data(DIS_TRAIN_FILE, index, uid)
                        index = index_new
            ...
            for g_epoch in range(50):  # 50
                for u in user_pos_train:
                    ...
                    ###########################################################################
                    dis_objective = sess.run(discriminator.objectiveF, {discriminator.u: u, discriminator.i: sample, discriminator.label: rating_sample})
                    fui_objective = sess.run(discriminator.fui, {discriminator.u: u, discriminator.i: sample, discriminator.label: rating_sample})
                    reward = sess.run(discriminator.reward, {discriminator.u: u, discriminator.i: sample, discriminator.label: rating_sample})
